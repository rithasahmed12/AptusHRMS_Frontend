...
const EditPlansModal = ({ isOpen, onClose, plan }: { isOpen: boolean; onClose: () => void; plan: any }) => {
                        ...
                        <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div className="sm:flex sm:items-start mt-2">
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <h3 className="text-lg leading-6 font-medium text-gray-900 mb-8 text-center">Edit Plan</h3>
                                    <div className="mb-4 flex flex-wrap justify-between gap-4">
                                        <div className="w-48">
                                            <label htmlFor="name" className="block text-sm font-medium text-gray-700">Plan Name</label>
                                            <input type="text" value={planName} onChange={(e) => setPlanName(e.target.value)} className="mt-1 mb-2 ps-1 block w-full h-8 shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        </div>
                                        <div className="w-48">
                                            <label htmlFor="price" className="block text-sm font-medium text-gray-700">Pricing(₹)</label>
                                            <input type="text" value={planAmount} onChange={(e) => setPlanAmount(e.target.value)} className="mt-1 mb-2 ps-1 block w-full h-8 shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        </div>
                                    </div>
                                    <div className="mb-4 flex flex-wrap justify-between">
                                        <div className="w-48">
                                            <label htmlFor="duration" className="block text-sm font-medium text-gray-700">Duration(in days)</label>
                                            <input type="text" value={planDuration} onChange={(e) => setPlanDuration(e.target.value)} className="mt-1 mb-2 ps-1 block w-full h-8 shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        </div>
                                        <div className="w-48">
                                            <label htmlFor="employees" className="block text-sm font-medium text-gray-700">Max Employees</label>
                                            <input type="text" value={maxEmployees} onChange={(e) => setMaxEmployees(e.target.value)} className="mt-1 mb-2 ps-1 block w-full h-8 shadow-sm sm:text-sm border-gray-300 rounded-md" />
                                        </div>
                                    </div>
                                </div>
                            </div>
